...
# 어떤 두 소도 길 위의 같은 점을 지나가지 않는다. 이걸 지켜본 존은 이 점들을 분석해 보기로 했다. 소는 총 26마리고, A, B, ... Z라는 이름이 붙는다.
# 존은 52개의 점을 시계방향으로 보면서 각 점을 어떤 소가 지나가는지 기록했다. 이렇게 만들어 낸 52글자의 문자열에는 각 알파벳이 두 번씩 나타날 것이다.
...
array = [[] for _ in range(52)]
temp = sys.stdin.readline().rstrip()
result = 0

for i in range(52):
    value = ord(temp[i])-ord('A')
    array[value].append(i)

for i in range(26):
    for j in range(i+1,26):
        if array[i][0] < array[j][0] < array[i][1] < array[j][1] or array[j][0] < array[i][0] < array[j][1] < array[i][1]:
            result += 1
