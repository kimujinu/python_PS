...
# 호주에 도착하고 처음 며칠은 한국 생각을 잊으면서 즐겁게 지냈다.
# 몇 주가 지나니 한국이 그리워지기 시작했다.
...
for _ in range(N):
    data = input()
    if data[:len(pattern[0])] == pattern[0] and data[-len(pattern[1]):] == pattern[1] and len("".join(pattern)) <=len(data):
        print("DA")
    else:
        print("NE")
