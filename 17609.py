...
# 또는 한 문자를 삭제하면 회문이 되는 “유사회문”인지, 아니면 회문이나 유사회문도 아닌 일반 문자열인지를 판단해야 한다.
...
def similar(start,end,level):
    flag = level
    while start < end:
        if data[start] != data[end] and level == 0:
           a = similar(start+1,end,level+1) # 앞 뛰어넘기
           b = similar(start,end-1,level+1) # 뒤 뛰어넘기
           flag = min(a,b)
           return flag
        elif data[start] != data[end] and level == 1: # 한개 뛰어넘기
            return 2
        else:
            start += 1
            end -= 1
