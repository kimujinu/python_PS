...
# 위의 그림은 상공에서 본 상근이네 집의 건물 배치이다. 정육각형 안의 숫자는 좌표를 나타낸다.
# 여기서 회색 정육각형은 건물의 위치이고, 흰색은 건물이 없는 곳이다. 위에서 붉은 색 선으로 표시된 부분이 밖에서 보이는 벽이고, 이 벽에 조명을 장식할 것이다. 벽의 총 길이는 64미터이다.
...
graph = [[0]*(h+2) for _ in range(w+2)]
vis = [[0]*(h+2) for _ in range(w+2)]
for i in range(1,h+1):
    temp_list = list(map(int,input().split()))
    temp_list.reverse()
    for j in range(1,w+1):
        graph[j][i] = temp_list.pop()
