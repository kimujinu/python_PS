...
baby_shark_level = 2
baby_shark_loc_x,baby_shark_loc_y = 0,0
eat = 0
sec = 0
result = 0
...
def bfs(x,y,time,value):
    ...
    while queue:
        ...
        if temp:
            nx, ny = min(temp)
            value += 1
            if value == baby_shark_level:
                value = 0
                baby_shark_level += 1
            graph[nx][ny] = 0
            return nx, ny, vis[x][y] + 1, value
...
for i in range(N):
    for j in range(N):
        if graph[i][j] == 9:
            baby_shark_loc_x,baby_shark_loc_y = i,j

graph[baby_shark_loc_x][baby_shark_loc_y] = 0

while True:
    baby_shark_loc_x,baby_shark_loc_y,sec,eat = bfs(baby_shark_loc_x,baby_shark_loc_y,sec,eat)
