...
pack = sorted(data,key=lambda x:x[0])
one_pack = sorted(data,key=lambda x:x[1])

if pack[0][0] <= one_pack[0][1]*6:
    result = pack[0][0] * (N//6) + one_pack[0][1] * (N%6)
    if pack[0][0] < one_pack[0][1] * (N%6):
        result = pack[0][0] * (N//6 + 1)
else:
    result = one_pack[0][1] * N
